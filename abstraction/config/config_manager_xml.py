...
class ConfigManagerXml(ConfigManager):
    ...
    def read_config(self, filename: str):
        with open(ConfigManager.CONFIG_FOLDER + filename + ConfigManagerXml.FILE_SUFFIX) as fd:
    ...
    def write_config(self, filename: str, config_data: dict):
        ...
        xmlfile = open(ConfigManager.CONFIG_FOLDER + filename + ConfigManagerXml.FILE_SUFFIX, 'w')
        xmlfile.write(xml)
        xmlfile.close()